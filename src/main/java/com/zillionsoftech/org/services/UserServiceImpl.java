...
import com.zillionsoftech.org.repositories.RolesRepository;
import com.zillionsoftech.org.repositories.UsersRepository;
...
public class UserServiceImpl implements UsersService, UserDetailsService {
	...
	public Users signUp(final Users user) {
		...
		user.setUserCreatedDate(Instant.now().toEpochMilli());
	...
	private List<GrantedAuthority> getUserAuthority(Set<Roles> userRoles) {
		...
		for (Roles role : userRoles) {
			roles.add(new SimpleGrantedAuthority(String.valueOf(role.getRoleId())));
		}
